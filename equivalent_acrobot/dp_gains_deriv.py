...
alpha1_virt = solve(eom[0], [alpha1])[0]

alpha2_virt = solve(eom[1], [alpha2])[0]
...
alpha1_virt_other = simplify(expand(alpha1_virt) - expand(alpha2*alpha1_virt_a2))
alpha2_virt_other = simplify(expand(alpha2_virt) - expand(alpha2*alpha2_virt_a2))

alpha_virt_mat = Matrix([[alpha1_virt_a2, alpha1_virt_other], [1, alpha2_virt_other]])
alpha_virt_mat_padded = alpha_virt_mat.col_join(Matrix([[0, 1]]))
...
alpha_cdd_mat_padded = alpha_cdd_mat.col_join(Matrix([[0, 1]]))

tcdd_alpha_cdd_mat = simplify(t_c_ddot_mat*alpha_cdd_mat_padded)

alpha2_reverse_mat = Matrix([[1/tcdd_alpha_cdd_mat[0], -tcdd_alpha_cdd_mat[1]/tcdd_alpha_cdd_mat[0]]])
alpha2_reverse_mat_coefs = Matrix([des_com_ang_acc, 1])
alpha2_reverse_mat_padded = alpha2_reverse_mat.col_join(Matrix([[0, 1]]))

alpha2_reverse_dict = dict(zip([alpha2], alpha2_reverse_mat*alpha2_reverse_mat_coefs))
zero_force = dict(zip([F_com_x, F_com_y], [0, 0]))
inv_dyn = eom.subs(zero_force)

inv_dyn_2 = inv_dyn[1].subs(alpha1_virt_dict)

t2 = inv_dyn_2.subs(alpha2_reverse_dict)
