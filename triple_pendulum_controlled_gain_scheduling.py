...
initial_coordinates = array([-0.2,1.2,1.2])
#1.495,3.13
...
inputKcheat = open('triple_pen_LQR_K_zoom_cheat.pkl','rb')
...
def zero_controller(x, t):
  ...
  if(counter == 0):
    ...
    print(lastidx)
  if(x[3] < 1 and x[3] > -1):
    idx = np.abs(angle_1 - (x[0] + 0.01)).argmin()
...
C_x = LA_x + numerical_constants[5]*sin(y[:,1] + y[:,0])
C_y = LA_y + numerical_constants[5]*cos(y[:,1] + y[:,0])

M_x = C_x + numerical_constants[11]*sin(y[:,1] + y[:,0] - 1.57)
M_y = C_y + numerical_constants[11]*cos(y[:,1] + y[:,0] - 1.57)

RH_x = LA_x + numerical_constants[4]*sin(y[:,1] + y[:,0])
RH_y = LA_y + numerical_constants[4]*cos(y[:,1] + y[:,0])

RA_x = RH_x + numerical_constants[8]*2*sin(y[:,2] + y[:,1] + y[:,0])
RA_y = RH_y + numerical_constants[8]*2*cos(y[:,2] + y[:,1] + y[:,0])
...
ani.save('triple_pendulum_zero_controller_another_one.mp4')
plt.show()
